    ...
    <a-card :bordered="false">
        <a-form @submit="handleSubmit" :form="form">
            <a-tabs default-active-key="1">
                ...
                <a-tab-pane key="4" tab="第四次测试值" force-render>
                    <a-form-item label="轨迹数据文件" :labelCol="labelCol" :wrapperCol="wrapperCol">
                        <j-upload v-decorator="['file4', validatorRules.file]" :trigger-change="true"></j-upload>
                        <a @click="handleExportPointCloud('file4')">导出点云</a>
                    </a-form-item>
                    <a-form-item label="起点序号" :labelCol="labelCol" :wrapperCol="wrapperCol">
                        <a-input-number style="width: 100%" v-decorator="['a4', { rules: [{ required: true, message: '请输入起点' }] }]" name="b4" placeholder="请输入起点" />
                    </a-form-item>
                    <a-form-item label="终点序号" :labelCol="labelCol" :wrapperCol="wrapperCol">
                        <a-input-number style="width: 100%" v-decorator="['b4', { rules: [{ required: true, message: '请输入终点' }] }]" name="b4" placeholder="请输入终点" />
                    </a-form-item>
                </a-tab-pane>
            </a-tabs>
...
export default {
    ...
    methods: {
        ...
        analysis(data) {
            httpAction(`/uav/analysis/pfs`, data, 'post')
                .then((res) => {
                    if (res.success) {
                        if (res.result !== null) {
        ...
        init() {
            ...
            httpAction(`/uav/uavTaskItem/queryByTypeAndTask?taskId=${taskId}&type=4`, {}, 'get')
                .then((res) => {
                    if (res.success) {
